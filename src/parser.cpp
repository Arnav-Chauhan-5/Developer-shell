...
std::vector<std::string> tokenize(const std::string &input) {
  ...
  // Treat \r and \n as whitespace so that Windows-style line endings
  // (and any stray newlines from FTXUI) are properly stripped from tokens.
  auto is_ws = [](char ch) {
  ...
  if (!current_token.empty() || state == State::InDouble ||
      state == State::InSingle) {
    tokens.push_back(std::move(current_token));
  }
